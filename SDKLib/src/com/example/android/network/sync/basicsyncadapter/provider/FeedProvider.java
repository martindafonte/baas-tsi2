...
public class FeedProvider extends ContentProvider {
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (uriMatch) {
            case ROUTE_ENTRIES_ID:
                ...
                builder.where(BaasContract.ColaSinc._ID + "=?", id);
            case ROUTE_ENTRIES:
                // Return all known entries.
                builder.table(BaasContract.ColaSinc.TABLE_NAME)
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match) {
            case ROUTE_ENTRIES:
                long id = db.insertOrThrow(BaasContract.ColaSinc.TABLE_NAME, null, values);
                result = Uri.parse(BaasContract.ColaSinc.COLUMN_NAME_ITEM_ID + "/" + id);
                break;
            case ROUTE_ENTRIES_ID:
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case ROUTE_ENTRIES:
                count = builder.table(BaasContract.ColaSinc.TABLE_NAME)
                        .where(selection, selectionArgs)
                        .delete(db);
                break;
            case ROUTE_ENTRIES_ID:
                String id = uri.getLastPathSegment();
                count = builder.table(BaasContract.ColaSinc.TABLE_NAME)
                       .where(BaasContract.ColaSinc._ID + "=?", id)
                       .where(selection, selectionArgs)
                       .delete(db);
                break;
            default:
    ...
    static class FeedDatabase extends SQLiteOpenHelper {
        ...
        private static final String SQL_CREATE_ENTRIES =
                "CREATE TABLE " + BaasContract.ColaSinc.TABLE_NAME + " (" +
                		BaasContract.ColaSinc._ID + " INTEGER PRIMARY KEY," +
                		BaasContract.ColaSinc.COLUMN_NAME_ITEM_ID + TYPE_TEXT + COMMA_SEP +
                        BaasContract.ColaSinc.COLUMN_NAME_ACCION    + TYPE_TEXT + COMMA_SEP +
                        BaasContract.ColaSinc.COLUMN_NAME_JSON + TYPE_TEXT + COMMA_SEP +
                         ")";

        /** SQL statement to drop "entry" table. */
        private static final String SQL_DELETE_ENTRIES =
                "DROP TABLE IF EXISTS " + BaasContract.ColaSinc.TABLE_NAME;
